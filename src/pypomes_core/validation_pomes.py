...
from .enum_pomes import EnumUseName, EnumUseAny, IntStrEnum
...
def validate_enum(source: dict[str, Any],
                  ...
                  required: bool = False,
                  errors: list[str] = None,
                  logger: Logger = None) -> IntStrEnum | None:
    """
    ...
    value: Any = None
    vals: list[str | int | Decimal] = []
    # HAZARD: do not use 'enum_class.__members__'
    values = values or [v for v in enum_class]
    for v in values:
        if isinstance(v, EnumUseName):
            vals.append(v.name.lower())
        elif isinstance(v, EnumUseAny):
            # noinspection PyUnresolvedReferences
            vals.append(v.anyval)
    ...
    if issubclass(enum_class, StrEnum):
        ...
        value: int = validate_int(source=source,
                                  ...
                                  logger=logger)
    if value:
        if issubclass(enum_class, EnumUseAny):
            # noinspection PyUnresolvedReferences
            result = enum_class.from_anyval(value)
        elif issubclass(enum_class, EnumUseName):
...
def validate_enums(source: dict[str, Any],
                   attr: str,
                   enum_class: type[IntEnum | StrEnum],
                   sep: str = ",",
                   required: bool = False,
                   errors: list[str] = None,
                   logger: Logger = None) -> list[IntStrEnum] | None:
    """
    Validate the *enum* values associated with *attr* in *source*.

    If provided, these values must be names or values corresponding to instances of *enum_class*.
    The only accepted values for *enum_class* are subclasses of  *StrEnum* or *IntEnum*.
    ...
    """
    # initialize the return variable
    result: list[IntEnum | StrEnum] | None = None

    values: list[int | str] | None = None
    if issubclass(enum_class, StrEnum):
        values: list[str] = validate_strs(source=source,
                                          attr=attr,
                                          sep=sep,
                                          required=required,
                                          errors=errors,
        ...
        values: list[int] = validate_ints(source=source,
                                          attr=attr,
                                          sep=sep,
                                          required=required,
                                          errors=errors,
...
def validate_format_error(__error_id: int,
                          /,
                          *args: Any,
    ...
    *VALIDATION_MSG_LANGUAGE* and *VALIDATION_MSG_PREFIX*, may be provided in *msg_lang* and *msg_prefix*.
    ...
    :param __error_id: the identification of the message element
    :param args: optional non-keyworded arguments to format the error message with
    :param msg_lang: the error message language to use (uses the default language, if not specified)
    :param msg_prefix: the error code prefix to use (uses the default prefix, if not specified)
    ...
    if __error_id != 100:
        if msg_prefix:
            result += msg_prefix + str(__error_id) + ": "
        result += err_msgs.get(__error_id, "")
...
def validate_format_errors(__errors: list[str],
                           /,
                           msg_lang: MsgLang = VALIDATION_MSG_LANGUAGE,
    ...
    *VALIDATION_MSG_LANGUAGE* and *VALIDATION_MSG_PREFIX*, may be provided in *msg_lang* and *msg_prefix*.
    ...
    :param __errors: the list of errors to build the list of *dicts* with
    :param msg_lang: the error message language to use (uses the default language, if not specified)
    :param msg_prefix: the error code prefix to use (uses the default prefix, if not specified)
...
def validate_unformat_errors(__errors: list[dict[str, str] | str],
                             /,
                             msg_lang: MsgLang = VALIDATION_MSG_LANGUAGE) -> list[str]:
    """
    Extract and return the list of errors used to build the list of dicts *errors*.

    Optional custom language, replacing the one defined by the environment variable*VALIDATION_MSG_LANGUAGE*,
    may be provided in *msg_lang*.

    :param __errors: the list of dicts to extract the errors from
    :param msg_lang: the error message language to use (uses the default language, if not specified)
